...
update `#__cot_admin` set `observation_datetime`=STR_TO_DATE(CONCAT('01/',`observation_month`,'/',`observation_day`), '%d/%m/%Y') where `observation_day`=0 and `observation_month`<>0 and `observation_year`<>0;
...
update `#__cot_admin` set `observation_datetime`=STR_TO_DATE(CONCAT('01/01/2017'), '%d/%m/%Y') where `observation_day`=0 and `observation_month`=0 and `observation_year`=0;
